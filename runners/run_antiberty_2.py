...
def main() -> None:
    ...
    embeddings = runner.embed([sequence])[0].detach().cpu().numpy()

    # AntiBERTy may return 2 extra special-token embeddings
    if embeddings.shape[0] == len(sequence) + 2:
        ...
        raise ValueError(
            f"Unexpected embedding length {embeddings.shape[0]} for sequence length {len(sequence)}"
        )
